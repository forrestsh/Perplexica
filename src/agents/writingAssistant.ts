...
const convertMessages = (value, index, array) => {
  var role;
  if (value["role"] === "user") role="human";
  else if (value["role"] === "assistant") role="ai";
  else if (value["role"] === "system") role="system";
  else role="generic";
  return [role, value["content"]];
};

function loadHandbook( file_name ){
  const mq_qa_zh_Hans = JSON.parse( fs.readFileSync( file_name, 'utf8').toString());
